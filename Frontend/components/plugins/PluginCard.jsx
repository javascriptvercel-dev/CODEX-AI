    ...
    >
      ...
      <div className="mt-2">
        ...
        <button
          type="button"
          onClick={copyCommand}
          disabled={!plugin.installCommand}
          className="focus-ring flex w-full items-center justify-center gap-1.5 rounded-lg border border-edge bg-surface2 py-2 text-xs font-semibold transition hover:border-azure-500/60 disabled:cursor-not-allowed disabled:opacity-50"
        >
          
          {copied ? (
            <Check size={13} className="text-green-400" />
          ) : (
            <Terminal size={13} className="text-azure-500" />
          )}
