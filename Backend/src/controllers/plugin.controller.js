...
export const getPluginRaw = async (req, res) => {
  const { data, error } = await supabase
    ...
    .maybeSingle();
  if (error) {
    console.error("getPluginRaw failed", error);
    return res.status(500).send("Could not load that plugin.");
  }
  if (!data) return res.status(404).send("Plugin not found.");
  res.type("text/plain; charset=utf-8").send(data.code || "");
};
